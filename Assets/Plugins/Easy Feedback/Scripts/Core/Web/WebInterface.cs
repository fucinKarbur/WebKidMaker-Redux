...
#pragma warning restore 0618
		...
		{
			// create the request
			UnityWebRequest request = new UnityWebRequest(uri, _getRequestMethodString(method));
			...
			{
				request.SetRequestHeader(Constants.Web.Header.ContentType, contentType);
			}
		...
		private static AsyncOperation _sendWebRequest(UnityWebRequest request)
		{
			...
			return (AsyncOperation)request.SendWebRequest();
#else
			return request.Send();
#endif
		}
		...
		private static string _getRequestMethodString(WebRequestMethod method)
		{
			switch (method)
			{
				case WebRequestMethod.GET: return Constants.Web.RequestMethod.GET;
				case WebRequestMethod.POST: return Constants.Web.RequestMethod.POST;
				case WebRequestMethod.PUT: return Constants.Web.RequestMethod.PUT;
				default: throw new System.NotImplementedException("No string conversion for provided WebRequestMethod value.");
			}
		}

		/// <summary>
		/// Checks if the ServerCertificateValidatiomCallback is set yet, and sets it if not.
		/// </summary>
